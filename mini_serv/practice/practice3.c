...
{
	for (int rec = 0; rec <= largest_fd; ++rec)
	{
		if (rec == sockfd || rec == fd || !FD_ISSET(rec, &all_fds))
			continue ;

		send(rec, msg, strlen(msg), 0);
	}
}
...
void	accept_client()
{
	...
	sprintf(msg, "client %d just joined", uids[fd]);
...
void	disconnect_client(int fd)
{
	...
	sprintf(msg, "client %d just left", uids[fd]);
...
int	recv_from_client(int fd)
{
	...
	int	amount = recv(fd, buf, buffer_size - 1, 0);
...
int main(int argc, char* argv[]) {
	...
	while (1)
	{
		...
		if (select(largest_fd + 1, &rfds, NULL, NULL, NULL) < 1)
			continue ;
		...
		for (int fd = 0; fd <= largest_fd; ++fd)
		{
			...
			while (1)
			{
				...
				char*	prefix = (char*)calloc(42, sizeof(char));
